function micromouse(maze)
    ...
    while ~isequal(mouse.cell, goal)
        ...
        boost = obterBoost(consecutivos(trecho),cellPercorridas,config.boost_tipo,ordens(consecutivos(trecho)));

        % Obtendo leitura dos sensores
        sensores= obterInfravermelho(coord, paredesHV);

        % Chamando o PID
        [vR,vL, ctrl] = ctrl.update(coord,mouse, linha, dt, sensores, boost, config.lookahead,config.ld_show,ax);
    ...
    if strcmp(config.graficos,'true')
        plotaGraficos(time_vec,vel_ref,vel_real,lookds,boosts,w_ref,w_real,giroL_ref,giroL_real);
    end
end
